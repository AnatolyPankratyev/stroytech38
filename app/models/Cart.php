    ...
    public function addToCart($product, $qty = 1, $mod = null){
        ...
        if(isset($_SESSION['cart'][$ID])){
            ...
            $_SESSION['cart'][$ID] = [
                ...
                'title' => $title,
                'alias' => $product->alias,
                'price' => $price * $_SESSION['cart.currency']['value'] * $price_index,
                'img' => $product->img,
            ];
        }
        $_SESSION['cart.qty'] = isset($_SESSION['cart.qty']) ? $_SESSION['cart.qty'] + $qty : $qty;
        $_SESSION['cart.sum'] = isset($_SESSION['cart.sum']) ? $_SESSION['cart.sum'] + $qty * ($price * $_SESSION['cart.currency']['value'] * $price_index) : $qty * ($price * $_SESSION['cart.currency']['value'] * $price_index);
    }
